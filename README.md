...
| `getProjects()`                      | Lista projetos distintos em ordem alfabética            |
...
Os testes não requerem arquivo de banco nem `sqlite3.dll` no PATH — usam SQLite in-memory (`DataBaseHelper.initMemory`).

| Arquivo                      | O que cobre                                                               |
| ---------------------------- | ------------------------------------------------------------------------- |
| `test/log_entry_test.dart`   | `toMap`, `fromMap`, nullable, round-trip, `withId`                        |
| `test/log_service_test.dart` | `insert`, `getAll`, `delete`, `update`, `search`, `filter`, `getProjects` |
